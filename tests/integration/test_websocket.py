...
async def test_websocket_auth_handshake(user_factory):
    """Verifies authentication enforcement and handshake behavior for incoming WebSocket connections."""
...
async def test_websocket_direct_message_realtime_push(client, user_factory):
    """Tests real-time WebSocket frame delivery for direct messages sent to an active recipient session."""
...
async def test_websocket_group_message_realtime_push(client, user_factory):
    """Tests group topic broadcast delivery to active member WebSocket connections."""
...
async def test_websocket_revoking_group_membership_blocks_pushes(client, user_factory):
    """Verifies that revoking group membership updates routing state and halts subsequent group pushes."""
    ...
    with patch.object(websocket_service, "publish_ws_push", side_effect=mock_publish):
        async with websocket_connect(f"/ws?token={bob.tokens.access_token}") as bob_ws:
            ...
            # Assert Bob receives the eviction event generated by _handle_membership_change
            eviction_event = await bob_ws.receive_json()
            assert eviction_event["event"] == "group_membership"
            assert eviction_event["group_id"] == str(group_id)
